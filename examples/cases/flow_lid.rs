...
///
/// Geometry:
/// Square domain in the xz plane with smaller tet elements near the walls and extruded in y.
///
/// Domains:
/// 0 - main domain (1.0 m x 0.2 m x 1.0 m)
/// 
/// Boundaries:
/// 0 - left boundary; -x (1.0 m x 0.2 m)
/// 1 - right boundary; +x (1.0 m x 0.2 m)
/// 2 - front boundary; -y (1.0 m x 1.0 m)
/// 3 - back boundary; +y (1.0 m x 1.0 m)
/// 4 - bottom boundary; -z (1.0 m x 0.2 m)
/// 5 - top boundary; +z (1.0 m x 0.2 m)
///
/// Properties:
/// dom_0 - density (rho = 1000.0 kg m-3)
/// dom_0 - viscosity (mu = 0.001 Pa s)
/// dom_0 - body force (f = <0.0, 0.0, 0.0> N m-3)
///
/// Boundary conditions:
/// bnd_0 - no-slip (v = <0.0, 0.0, 0.0> m s-1)
/// bnd_1 - no-slip (v = <0.0, 0.0, 0.0> m s-1)
/// bnd_2 - no-slip (v = <0.0, 0.0, 0.0> m s-1)
/// bnd_3 - no-slip (v = <0.0, 0.0, 0.0> m s-1)
/// bnd_4 - no-slip (v = <0.0, 0.0, 0.0> m s-1)
/// bnd_5 - velocity (v = <4.0e-4, 0.0, 0.0> m s-1)
...
fn main() -> Result<(), FEChemError> {
    ...
    let bnd_f = vars.add_bnd(dom, 2)?;  // front
    let bnd_k = vars.add_bnd(dom, 3)?;  // back
    let bnd_b = vars.add_bnd(dom, 4)?;  // bottom
    let bnd_t = vars.add_bnd(dom, 5)?;  // top

    // unknown domain vectors
    // arguments: domain, initial_value_x, initial_value_y, initial_value_z, output_file
    ...
    let vel = vars.add_vecdom_unk(dom, 0.0, 0.0, 0.0, "examples/output_flow_lid/vel.vtu".to_string())?;
    ...
    // arguments: domain, value_x, value_y, value_z, output_file
    let fce = vars.add_vecdom_con(dom, 0.0, 0.0, 0.0, "".to_string())?;  // body force (den * g; not acceleration g)

    // constant boundary vectors
    // arguments: boundary, value_x, value_y, value_z, output_file
    let vel_l = vars.add_vecbnd_con(bnd_l, 0.0, 0.0, 0.0, "".to_string())?;  // velocity
    let vel_r = vars.add_vecbnd_con(bnd_r, 0.0, 0.0, 0.0, "".to_string())?;  // velocity
    let vel_f = vars.add_vecbnd_con(bnd_f, 0.0, 0.0, 0.0, "".to_string())?;  // velocity
    let vel_k = vars.add_vecbnd_con(bnd_k, 0.0, 0.0, 0.0, "".to_string())?;  // velocity
    let vel_b = vars.add_vecbnd_con(bnd_b, 0.0, 0.0, 0.0, "".to_string())?;  // velocity
    let vel_t = vars.add_vecbnd_con(bnd_t, 4.0e-4, 0.0, 0.0, "".to_string())?;  // velocity
    ...
    phys.add_vel_bnd(bnd_f, vel_f);  // arguments: boundary, vel
    phys.add_vel_bnd(bnd_k, vel_k);  // arguments: boundary, vel
