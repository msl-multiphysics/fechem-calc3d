...
///
/// Geometry:
/// Channel with bump surrounded by solid regions, extruded in y.
///
/// Domains:
/// 0 - channel
/// 1 - bottom solid
...
/// bnd_6 - no flux (q = 0.0 W m-2)
/// bnd_7 - no flux (q = 0.0 W m-2)
///
...
///
/// Boundary conditions:
/// bnd_0 - velocity (v = <0.002, 0.0, 0.0> m s-1)
/// bnd_1 - pressure (p = 0.0 Pa)
/// bnd_4 - no-slip (v = <0.0, 0.0, 0.0> m s-1)
/// bnd_5 - no-slip (v = <0.0, 0.0, 0.0> m s-1)
/// bnd_6 - no-slip (v = <0.0, 0.0, 0.0> m s-1)
/// bnd_7 - no-slip (v = <0.0, 0.0, 0.0> m s-1)
/// 
fn main() -> Result<(), FEChemError> {
    ...
    let bnd_cf = vars.add_bnd(dom_c, 6)?;  // channel front
    let bnd_ck = vars.add_bnd(dom_c, 7)?;  // channel back
    ...
    let hflx_cf = vars.add_sclbnd_con(bnd_cf, 0.0, "".to_string())?;  // heat flux
    let hflx_ck = vars.add_sclbnd_con(bnd_ck, 0.0, "".to_string())?;  // heat flux
    ...
    // initial_value - initial guess for steady-state problems; initial_value for transient problems
    // output_file - can be .csv or .vtu; if empty string, no file is written
    let vel = vars.add_vecdom_unk(dom_c, 0.0, 0.0, 0.0, "examples/output_heatflow_heater/vel.vtu".to_string())?;
    ...
    let fce = vars.add_vecdom_con(dom_c, 0.0, 0.0, 0.0, "".to_string())?;  // body force (den * g; not acceleration g)

    // constant boundary vectors
    // arguments: boundary, value_x, value_y, value_z, output_file
    let vel_i = vars.add_vecbnd_con(bnd_i, 0.002, 0.0, 0.0, "".to_string())?;  // velocity
    let vel_bc = vars.add_vecbnd_con(bnd_bc, 0.0, 0.0, 0.0, "".to_string())?;  // velocity
    let vel_tc = vars.add_vecbnd_con(bnd_tc, 0.0, 0.0, 0.0, "".to_string())?;  // velocity
    let vel_cf = vars.add_vecbnd_con(bnd_cf, 0.0, 0.0, 0.0, "".to_string())?;  // velocity
    let vel_ck = vars.add_vecbnd_con(bnd_ck, 0.0, 0.0, 0.0, "".to_string())?;  // velocity
    ...
    phys.add_hflx_bnd(bnd_cf, hflx_cf);  // arguments: boundary, q
    phys.add_hflx_bnd(bnd_ck, hflx_ck);  // arguments: boundary, q
    ...
    phys.add_vel_bnd(bnd_cf, vel_cf);  // arguments: boundary, v
    phys.add_vel_bnd(bnd_ck, vel_ck);  // arguments: boundary, v
